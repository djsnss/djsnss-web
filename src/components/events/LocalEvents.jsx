...
const LocalEvents = () => {
  ...
  const [localEventsData, setLocalEventsData] = useState([]);
    ...
    useEffect(()=>{
      const fetchData = async()=>{
        ...
        setLocalEventsData(formattedEvents)
      };
  ...
  return (
    <div className="h-[60vh] sm:h-[80vh] w-full px-4 mt-12 mb-5">
      ...
      <CCarousel controls={localEventsData.length > 1}
          indicators={localEventsData.length > 1}
          interval={localEventsData.length > 1 ? 5000 : false}>
        {localEventsData.map((event) => (
