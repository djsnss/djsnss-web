...
const Sidebar = () => {
  ...
  return (
    <nav
      className={`absolute sm:relative sm:h-screen p-2 shadow-lg border-r border-b border-white/20 sm:border-hidden backdrop-blur-lg transition-all duration-500 ease-in-out ${
        open ? "h-[100vh] w-[100vw] sm:w-48" : "h-max w-12 sm:w-16"
      } z-50 font-poppins rounded-br-lg sm:rounded-none overflow-hidden`}
      style={{
      ...
      >
        {navigationLinks.map(({ Icon, title, path }) => (
          <Link
            ...
            className={`flex items-center no-underline p-2 rounded-lg transition-all duration-300 ${activeRoute === path ? "bg-indigo-100 text-indigo-600" : "text-white hover:text-black hover:bg-gray-100/40"}`}
          >
